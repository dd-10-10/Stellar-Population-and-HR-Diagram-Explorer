...
\subsection{Stellar Parameter Derivations}

To construct the HR diagrams, we transform raw Gaia DR3 observations into physical stellar properties using standard geometric and photometric relations \cite{GaiaData}. 

\subsubsection{Distance and Absolute Magnitude}
The distance $d$ in parsecs is calculated from the parallax $\varpi$\cite{Lutz1973}:
\begin{equation}
    d = \frac{1000}{\varpi}
\end{equation}

The G-band absolute magnitude ($M_G$) is derived by correcting for extinction ($A_G$) as defined in the Gaia pipeline \cite{GaiaData}:
\begin{equation}
    M_{G} = G - A_{G} + 5 \log_{10}(\varpi) - 10
\end{equation}
\subsection{Absolute Visual Magnitude}
The absolute visual magnitude ($M_V$) is calculated by correcting the apparent V-band magnitude for extinction ($A_V$) and distance using the parallax ($\varpi$):
\begin{equation}
    M_{V} = V - A_{V} + 5 \log_{10}(\varpi) - 10
\end{equation}

\subsubsection{Luminosity}
Using the reference solar absolute magnitude $M_{G,\odot} \approx 4.67$ \cite{Casagrande2018}, the stellar luminosity $L$ is calculated in solar units ($L_{\odot}$)\cite{IAU2015}:
\begin{equation}
    \log_{10} \frac{L}{L_{\odot}} = 0.4(M_{G,\odot} - M_{G})
\end{equation}
...
\bibitem{Fillioux2025} 
L. Fillioux, J. Silva-Rodríguez, et al. 
\textit{Are foundation models for computer vision good conformal predictors?}, 
Mar. 2025. arXiv:2412.06082. [cite: 166, 167]

\bibitem{Silva2025} 
J. Silva-Rodríguez, I. Ben Ayed, and J. Dolz. 
\textit{Conformal Prediction for Zero-Shot Models}. 
pages 19931-19941, 2025. [cite: 168]

\bibitem{GaiaData} 
Gaia Collaboration. 
\textit{Gaia-based HR Diagram and Stellar Properties: Definitions and Gaia Columns}. 
\bibitem{Casagrande2018} 
L. Casagrande and D. A. VandenBerg. 
\textit{On the use of Gaia magnitudes and color-magnitude diagrams}. 
Monthly Notices of the Royal Astronomical Society, 475(4):5023–5040, 2018.

\bibitem{IAU2015} 
E. E. Mamajek, et al. 
\textit{IAU 2015 Resolution B3 on Recommended Nominal Conversion Constants for Selected Solar and Planetary Properties}. 
arXiv:1510.07674 [astro-ph.SR], 2015.

\bibitem{Lutz1973} 
T. E. Lutz and D. H. Kelker. 
\textit{On the Use of Trigonometric Parallaxes for the Calibration of Luminosity Systems}. 
Publications of the Astronomical Society of the Pacific, 85(507):573, 1973.
\end{thebibliography}
